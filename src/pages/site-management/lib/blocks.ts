...
import { Index } from "@/pages/site-management/__registry__"
import { Project, ScriptKind, SourceFile, SyntaxKind } from "ts-morph"
import { z } from "zod"

import { highlightCode } from "@/pages/site-management/lib/highlight-code"
import { Style } from "@/pages/site-management/registry/registry-styles"
import { BlockChunk, blockSchema, registryEntrySchema } from "@/pages/site-management/registry/schema"
  ...
  const chunks = await Promise.all(
    entry.chunks?.map(async (chunk: BlockChunk) => {
      ...
      return {
        ...
        code: sourceFile
          .getText()
          .replace(/@\/pages\/site-management\/registry\/${style}\//g, "@/pages/site-management/components/"),
      }
    })
  )
...
async function _getBlockContent(name: string, style: Style["name"]) {
  ...
  code = code.replace(/@\/pages\/site-management\/registry\/${style}\//g, "@/pages/site-management/components/")
